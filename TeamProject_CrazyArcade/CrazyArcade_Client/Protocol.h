...
typedef struct Char_Info
{
	...
	Char_Info()
	{
		...
		iState = E_CHARACTER_STATE_END;
		iMoveState = E_MOVE_END;

		IsVisible = true;
		cKey = 0;
	}
	bool operator != (const Char_Info& _rInfo)
	{
		if (_rInfo.iState != iState
			|| vDir != _rInfo.vDir
			|| vPos != _rInfo.vPos
			|| iMoveState != _rInfo.iMoveState
			|| cKey != _rInfo.cKey
			|| IsVisible != _rInfo.IsVisible)
			return true;
		return false;
	}
}CHAR_INFO_MSG;
